<template>
  <div id="mlids">
    ...
    <v-dialog v-model="dialog" width="600px">
      <v-card class="pa-7">
        <v-card-text>
          <p v-if="selected[0]"><b>Name: </b>{{ selected[0].name }}</p>
          <p v-if="selected[0]"><b>Tel: </b>{{ selected[0].tel }}</p>
...
export default {
  ...
  methods: {
    ...
    copyTo(address) {
      ...
      this.snackbar = true;
      if (address == "") {
        this.message = "No free addresses";
        return;
      }
          ...
          if (res.status == 200) {
            ...
            if (localStorage.hm) {
              if (localStorage.hm == self.hm) {
                ...
                self.message = "Your number of leads has changed!";
                self.snackbar = true;
